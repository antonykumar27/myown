...
const ContentCreate = ({ teacherId, onSuccess }) => {
  // --- State Management ---
  ...
  const [createLesson, { isLoading, error }] = useCreateTextContentMutation();
  const {
    data: count,
    isLoading: isContentLoading,
    refetch: isContentRefetch,
    error: isError,
  } = useGetTextContentCountQuery(chapterId);
  useEffect(() => {
    if (count?.nextPageNumber) {
      setPageNumber(count.nextPageNumber);
    }
  }, [count]);
  // --- Check if mobile ---
  useEffect(() => {
